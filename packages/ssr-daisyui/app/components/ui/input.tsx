...
import { type TranslationFn } from '~/lib/i18n-utils.ts'
...
interface InputProps
  extends Omit<React.InputHTMLAttributes<HTMLInputElement>, 'size'>, VariantProps<typeof variants> {
  description?: string
  error?: {
    message?: string
  }
      ...
      <fieldset className="fieldset">
        ...
        {(error?.message || description) && (
          <p className="fieldset-label">
            {error?.message && (
              <span className="text-error">{translate(error.message, error.message)}</span>
            )}
