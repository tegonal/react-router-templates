...
- **[react-i18next](https://react.i18next.com)**: Internationalization framework for multi-language support, with typesafe translation keys extracted and generated by **[i18next-cli](https://github.com/i18next/i18next-cli)** (run `yarn i18n:extract`)
...
- **[Base UI](https://base-ui.com)**: Headless, accessible component primitives. Use DaisyUI classes alone for simple components; for complex, interactive ones (dropdowns, dialogs, menus, selects, …) build on Base UI for behavior and style it with DaisyUI classes
...
- **Eslint, Prettier**: Preconfigured with sensible defaults for TypeScript and React in
  `eslint.config.js` and `.prettierrc.mjs`. The configuration is maintained directly in the project
  (no shared preset dependency), so you can customize it freely.
...
yarn run i18n:extract # extract i18n keys and regenerate typesafe types
yarn run i18n:types # regenerate i18n types only
yarn run check # lint, format and type-check before committing
```
